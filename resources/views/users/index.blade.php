<x-app-layout>
        ...
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-cyan-10 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="w-full p-6 flex items-center justify-between text-gray-900 flex-col md:flex-row gap-2">
                    ...
                    {{-- Título --}}
                    <span class="text-xl font-semibold">{{ __("Listagem de Usuários") }}</span>

                    {{-- Formulário de Pesquisa e Botões --}}
                    <div class="w-full md:w-2/3 md:min-w-fit flex items-center space-x-4 justify-center">
                        <form method="GET" action="{{ route('users.index') }}" class="w-full md:w-2/3 md:min-w-fit flex flex-col sm:flex-row items-center space-x-4 gap-2">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                ...
                @forelse($users as $user)
                    {{--  Caso existam usuários, a lista será exibida --}}
                    <tr class="odd:bg-white even:bg-slate-50 hover:bg-cyan-100">
                        <th scope="row"
                            class="px-6 py-4 font-medium whitespace-nowrap text-gray-600">{{ $user->id }}</th>
                        <td class="px-6 py-4">{{ $user->name }}</td>
                        <td class="px-6 py-4">{{ $user->email }}</td>
                        <td class="px-6 py-4">{{ $user->created_at->format('d/m/Y H:i') }}</td>
                        <td class="px-6 py-4">{{ $user->updated_at->format('d/m/Y H:i') }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="9" class="bg-cyan-10 px-6 py-4 text-center text-gray-900 whitespace-nowrap">Nenhum usuário encontrado.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
        ...
        <div class="flex items-center justify-center min-h-screen">
            ...
            <div class="bg-cyan-10 rounded-lg overflow-hidden shadow-xl transform transition-all sm:max-w-lg sm:w-full">
                ...
                <div class="bg-violet-50 px-4 py-3 sm:px-6 flex justify-end space-x-3">
                    {{-- Botões de Editar --}}
                    <a id="editButton"
                       class="uppercase inline-flex justify-center rounded-md border border-transparent px-4 py-2 bg-violet-600 text-base font-medium text-white shadow-sm hover:bg-violet-700 sm:text-sm">
                        Editar
                    </a>

                    {{-- Botão de Fechar --}}
                    <button id="closeModalButton"
                            class="uppercase inline-flex justify-center rounded-md border border-transparent px-4 py-2 bg-gray-600 text-base font-medium text-white shadow-sm hover:bg-gray-700 sm:text-sm">
                        Fechar
                    </button>
                </div>
            </div>
    ...
    <script>
        function openModal(user) {
            ...
            document.getElementById('modal-created_at').innerText = new Date(user.created_at).toLocaleString('pt-BR');
            document.getElementById('modal-updated_at').innerText = new Date(user.updated_at).toLocaleString('pt-BR');

            {{-- Ajustando o link de edição --}}
            document.getElementById('editButton').href = `/users/${user.id}/edit`;
